...
await page.route('**/api/health', route => route.fulfill({ status: 200, headers: { 'content-type': 'application/json' }, body: JSON.stringify({ database: 'connected' }) }));
await page.route('**/health', route => route.fulfill({ status: 200, headers: { 'content-type': 'application/json' }, body: JSON.stringify({ database: 'connected' }) }));

for (const vp of viewports) {
  console.log('Viewport:', vp.name, `${vp.width}x${vp.height}`);
  await page.setViewportSize({ width: vp.width, height: vp.height });

  const origUrl = `${base}/index.html#/login`;
